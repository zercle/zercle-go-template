...
// It parses the ID from URL parameter, calls the usecase to delete the user,
...
func (h *Handler) Delete(c *echo.Context) error {
	...
	if err != nil {
		h.getLogger(c).Error().Err(err).Str("user_id", id).Msg("failed to delete user")
	...
	h.getLogger(c).Info().Str("user_id", id).Msg("user deleted successfully")
