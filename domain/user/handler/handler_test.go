...
func setupTestUserHandler(t *testing.T) (*userHandler, *usermock.MockUsecase, *echo.Echo) {
...
func TestUserHandler_GetProfile(t *testing.T) {
	...
	tests := []struct {
		...
		},
		{
			name: "user not found",
			setupMock: func() {
				mockUsecase.EXPECT().GetProfile(gomock.Any(), testUserID).Return(nil, repository.ErrUserNotFound)
			},
			userID:          testUserID.String(),
...
func TestUserHandler_UpdateProfile(t *testing.T) {
	...
	tests := []struct {
		...
		},
		{
			name: "user not found",
			setupMock: func() {
				mockUsecase.EXPECT().UpdateProfile(gomock.Any(), testUserID, gomock.Any()).Return(nil, repository.ErrUserNotFound)
			},
			userID:          testUserID.String(),
			requestBody:     `{"full_name":"Updated Name"}`,
			wantStatus:      http.StatusNotFound,
