...
// Suite holds the test environment resources.
type Suite struct {
...
func SetupWithRetry(t *testing.T, maxRetries int) *Suite {
	var suite *Suite
	var err error
...
// SkipIfPodman skips the test if running on Podman.
func SkipIfPodman(t *testing.T) {
	if containers.IsPodman() {
		t.Skip("skipping integration test on Podman (testcontainers-go compatibility issue)")
	}
}
...
func SetupTestEnv(t *testing.T) *Suite {
	return SetupSuite(t)
}
