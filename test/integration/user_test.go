...
func TestUserRepository_Integration(t *testing.T) {
	...
	t.Run("create and get user", func(t *testing.T) {
		repo := user.NewPostgresRepository(suite.DB)

		newUser, err := user.New("integration@example.com", "hashedpassword", "Test", "User")
	...
	t.Run("get by email", func(t *testing.T) {
		...
		// Create a user first
		newUser, err := user.New("getbyemail@example.com", "hashedpassword", "GetBy", "Email")
		require.NoError(t, err)
		created, err := repo.Create(ctx, newUser)
		require.NoError(t, err)
	...
	t.Run("update user", func(t *testing.T) {
		...
		// Create a user first
		newUser, err := user.New("update@example.com", "hashedpassword", "Original", "Name")
		require.NoError(t, err)
		created, err := repo.Create(ctx, newUser)
		require.NoError(t, err)
	...
	t.Run("delete user", func(t *testing.T) {
		...
		// Create a user first
		newUser, err := user.New("delete@example.com", "hashedpassword", "Delete", "Me")
		require.NoError(t, err)
		created, err := repo.Create(ctx, newUser)
		require.NoError(t, err)
	...
	t.Run("list users with pagination", func(t *testing.T) {
		...
		for i := range 5 {
			newUser, err := user.New("listuser"+string(rune('a'+i))+"@example.com", "hashedpassword", "List", "User")
			require.NoError(t, err)
	...
	t.Run("list users with status filter", func(t *testing.T) {
		...
		newUser, err := user.New("activefilter@example.com", "hashedpassword", "Active", "User")
		require.NoError(t, err)
		_, err = repo.Create(ctx, newUser)
		require.NoError(t, err)
	...
	t.Run("duplicate email error", func(t *testing.T) {
		...
		newUser1, err := user.New(email, "hashedpassword", "First", "User")
		require.NoError(t, err)
		_, err = repo.Create(ctx, newUser1)
		require.NoError(t, err)

		// Try to create another user with the same email
		newUser2, err := user.New(email, "hashedpassword2", "Second", "User")
...
func TestUserStatusTransitions_Integration(t *testing.T) {
	...
	// Create a user
	testUser, err := user.New("statustest@example.com", "hash", "Status", "Test")
	require.NoError(t, err)
	...
	updated, err := repo.Update(ctx, created)
	require.NoError(t, err)
	assert.Equal(t, user.StatusSuspended, updated.Status)

	// Reactivate user
	updated.Activate()
	updatedAgain, err := repo.Update(ctx, updated)
	require.NoError(t, err)
	assert.Equal(t, user.StatusActive, updatedAgain.Status)

	// Deactivate user
	updatedAgain.Deactivate()
	deactivated, err := repo.Update(ctx, updatedAgain)
	require.NoError(t, err)
	assert.Equal(t, user.StatusInactive, deactivated.Status)
}
