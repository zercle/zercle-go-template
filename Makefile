...
# ============================================
test-db-up:
	@echo "🐳 Starting test database using $(CONTAINER_RUNTIME)..."
	$(COMPOSE_CMD) -f $(COMPOSE_TEST_FILE) up -d
	@echo "⏳ Waiting for database to be ready..."
	@sleep 3
	@$(CONTAINER_RUNTIME) exec zercle-postgres-test pg_isready -U postgres || (echo "❌ Database not ready" && exit 1)
	@echo "✅ Test database ready on port 5433"

test-db-down:
	@echo "🛑 Stopping test database using $(CONTAINER_RUNTIME)..."
...
test-db-reset: test-db-down test-db-up
	@echo "♻️  Test database reset complete"
