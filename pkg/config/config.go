...
type AuthConfig struct {
	AccessTokenSecret  string        `mapstructure:"access_token_secret" env:"AUTH_ACCESS_TOKEN_SECRET"`
	RefreshTokenSecret string        `mapstructure:"refresh_token_secret" env:"AUTH_REFRESH_TOKEN_SECRET"`
	AccessTokenTTL     time.Duration `mapstructure:"access_token_ttl" env:"AUTH_ACCESS_TOKEN_TTL" envDefault:"15m"`
	RefreshTokenTTL    time.Duration `mapstructure:"refresh_token_ttl" env:"AUTH_REFRESH_TOKEN_TTL" envDefault:"7d"`
	Issuer             string        `mapstructure:"issuer" env:"AUTH_ISSUER" envDefault:"zercle-go-template"`
}
...
// ObservabilityConfig holds observability settings (metrics, tracing).
type ObservabilityConfig struct {
	MetricsEnabled  bool   `mapstructure:"metrics_enabled" env:"OBSERVABILITY_METRICS_ENABLED" envDefault:"false"`
	MetricsPort     int    `mapstructure:"metrics_port" env:"OBSERVABILITY_METRICS_PORT" envDefault:"9090"`
	TracingEnabled  bool   `mapstructure:"tracing_enabled" env:"OBSERVABILITY_TRACING_ENABLED" envDefault:"false"`
	TracingEndpoint string `mapstructure:"tracing_endpoint" env:"OBSERVABILITY_TRACING_ENDPOINT" envDefault:"localhost:4317"`
	ServiceName     string `mapstructure:"service_name" env:"OBSERVABILITY_SERVICE_NAME" envDefault:"zercle-go-template"`
}
...
func Load() (*Config, error) {
	...
	v.SetDefault("observability.service_name", "zercle-go-template")
	v.SetDefault("log.level", "info")
	v.SetDefault("log.format", "json")
	v.SetDefault("auth.access_token_ttl", "15m")
	v.SetDefault("auth.refresh_token_ttl", "168h") // 7 days
	v.SetDefault("auth.issuer", "zercle-go-template")
...
func (c *Config) Validate() error {
	...
	if c.Auth.RefreshTokenSecret == "" {
		return fmt.Errorf("auth.refresh_token_secret is required")
	}
	return nil
}
